...
```{r}
# dbGetQuery(con, "SELECT * FROM workflow LIMIT 5;")
cmd <- paste0("SELECT id, create_time FROM workflow;")
...
df1 <- workflows_by_month %>%
  mutate(stat_item="workflow",
         ...
         description=map2_chr(stat_item, month, function(x, y) {
           sprintf("Number of %s created for month %s", x, y)
         })) %>%
...
df2 <- jobs_by_month %>%
  mutate(stat_item="job",
         stat_data=n,
         description=map2_chr(stat_item, month, function(x, y) {
           sprintf("Number of %s executed for month %s", x, y)
         })) %>%
  select(month, stat_item, stat_data, description)

df3 <- datasets_by_month %>%
  mutate(stat_item="dataset_uploaded",
         stat_data=n,
         description=map2_chr(stat_item, month, function(x, y) {
...
df4 <- dataset_sizes_by_month %>%
  mutate(stat_item="dataset_uploaded_storage",
...
```{r}
email_output <- email_suffix_df %>% 
  mutate(month=previous_year_month,
         email_domain=domain,
         count=freq,
         description=map_chr(month, function(x) {
           paste("Number of users with the email_domain at the end of", x)
         })) %>%
...
```{r}
tool_output <- top_tool_df %>%
  mutate(month=last_year_month,
         count=freq,
         description=map_chr(month, function(x) {
           paste("Number of instances of tool usage at the end of", x)
         })) %>%
...
```{r}
ref_output <- merge(dna_ref_df, rna_ref_df, by="reference", all=TRUE) %>%
  mutate(month=previous_year_month,
         reference_genome=reference,
         count=map2_int(freq.x, freq.y, sum, na.rm=TRUE),
         description=map_chr(month, function(x) {
           paste("Number of times the the reference_genome has been used",
                 "in common alignment tools at the end of", x)
         })) %>%
